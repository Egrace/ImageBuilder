...
describe ImageProcessing::ImageBuilder do
  let(:initial_width) { 800 }
  let(:initial_height) { 600 }
  let(:builder) { ImageProcessing::ImageBuilder.new(width: initial_width, height: initial_height) }
  ...
  describe '#with_background_image' do
    ...
    context 'with only source argument' do
      it 'creates input image processor with 0% darken' do
        builder.with_background_image(source: sample_source)
        expect(builder.processors.last.darken).to eq '0%'
      end
    end
  end
